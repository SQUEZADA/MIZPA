<html>
    ...
    <body>
        ...
        $codeProd= $_POST['idbiblioteca'];
        $nameProd= $_POST['Nombre'];
        $cateProd= $_POST['descripcion'];
        $adminProd= $_POST['admin-name'];

        if(!$codeProd=="" && !$nameProd=="" && !$cateProd=="" && !$_FILES['imagenlibro']['name']==""){
            $verificar=  ejecutarSQL::consultar("select * from biblioteca where idbiblioteca='".$codeProd."'");
            $verificaltotal = mysqli_num_rows($verificar);
            if($verificaltotal<=0){
                if(move_uploaded_file($_FILES['imagenlibro']['tmp_name'],"../assets/img-biblioteca/".$_FILES['imagenlibro']['name'])){
                    if(consultasSQL::InsertSQL("biblioteca", "nombre, descripcion,imagenlibro", "'$nameProd','$cateProd','".$_FILES['imagenlibro']['name']."'")){
                       echo '
