<html>
    ...
    <body>
        ...
        $codeProd= $_POST['idcontactos'];
        $nameProd= $_POST['full_name'];
        $cateProd= $_POST['Correo'];
        $numprod= $_POST['telefono'];
        $adminProd= $_POST['admin-name'];

        if(!$codeProd=="" && !$nameProd=="" && !$cateProd=="" && !$numprod=="" && !$_FILES['imagencontactos']['name']==""){
            $verificar=  ejecutarSQL::consultar("select * from contactos where idcontactos='".$codeProd."'");
            $verificaltotal = mysqli_num_rows($verificar);
            if($verificaltotal<=0){
                if(move_uploaded_file($_FILES['imagencontactos']['tmp_name'],"../assets/img-contactos/".$_FILES['imagencontactos']['name'])){
                    if(consultasSQL::InsertSQL("contactos", "full_name, correo, telefono, imagencontactos", "'$nameProd','$cateProd','$numprod','".$_FILES['imagencontactos']['name']."'")){
                       echo '
                            <img src="../assets/img/correctofull.png" class="center-all-contens">
                            <br>
                            <h3>Novedad Agregada</h3>
