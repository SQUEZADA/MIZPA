<html>
    ...
    <body>
        ...
        $codeProd= $_POST['idnosotros'];
        $nameProd= $_POST['Titulo'];
        $cateProd= $_POST['Descripcion'];
        $adminProd= $_POST['admin-name'];

        if(!$codeProd=="" && !$nameProd=="" && !$cateProd=="" && !$_FILES['imagenNosotros']['name']==""){
            $verificar=  ejecutarSQL::consultar("select * from nosotros where idnosotros='".$codeProd."'");
            $verificaltotal = mysqli_num_rows($verificar);
            if($verificaltotal<=0){
                if(move_uploaded_file($_FILES['imagenNosotros']['tmp_name'],"../assets/img-nosotros/".$_FILES['imagenNosotros']['name'])){
                    if(consultasSQL::InsertSQL("nosotros", "Titulo, Descripcion, imagenNosotros", "'$nameProd','$cateProd','".$_FILES['imagenNosotros']['name']."'")){
                       echo '
