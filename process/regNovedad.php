<html>
    ...
    <body>
        ...
        $codeProd= $_POST['IdNovedad'];
        $nameProd= $_POST['Titulo'];
        $cateProd= $_POST['Descripcion'];
        $adminProd= $_POST['admin-name'];

        if(!$codeProd=="" && !$nameProd=="" && !$cateProd=="" && !$_FILES['imagen']['name']==""){
            $verificar=  ejecutarSQL::consultar("select * from Novedad where IdNovedad='".$codeProd."'");
            $verificaltotal = mysqli_num_rows($verificar);
            if($verificaltotal<=0){
                if(move_uploaded_file($_FILES['imagen']['tmp_name'],"../assets/img-novedades/".$_FILES['imagen']['name'])){
                    if(consultasSQL::InsertSQL("novedad", "Titulo, Descripcion, Imagen", "'$nameProd','$cateProd','".$_FILES['imagen']['name']."'")){
                       echo '
